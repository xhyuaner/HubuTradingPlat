...
func (s *GoodsServer) BatchGetGoodsByCates(ctx context.Context, req *proto.BatchCategoriesIdInfo) (*proto.GoodsListResponse, error) {
	goodsListResponse := &proto.GoodsListResponse{}
	var goods []model.Goods

	//调用where并不会真正执行sql 只是用来生成sql的 当调用find， first才会去执行sql，
	result := global.DB.Model(&model.Goods{}).Where("category_id IN ?", req.CategoryId).Find(&goods)
	for _, good := range goods {
		goodsInfoResponse := ModelToResponse(good)
		goodsListResponse.Data = append(goodsListResponse.Data, &goodsInfoResponse)
	}
...
func (s *GoodsServer) CreateGoods(ctx context.Context, req *proto.CreateGoodsInfo) (*proto.GoodsInfoResponse, error) {
	...
	//新增商品数据时，必须保证mysql和es中新增数据同时成功或者同时失败，数据一致性
	tx := global.DB.Begin()
	result := tx.Save(&goods) //执行save函数的同时会调用model.Goods的AfterCreate钩子函数将数据添加到es中
	if result.Error != nil {  //如果保存到mysql或者es有错误就回滚
		tx.Rollback()
