...
import (
	...
	"gorm.io/gorm"
	"shop-srv/inventory-srv/model"

	"shop-srv/inventory-srv/global"
	"shop-srv/inventory-srv/proto"
)
...
func (*InventoryServer) Sell(ctx context.Context, req *proto.SellInfo) (*emptypb.Empty, error) {
	...
	for _, goodInfo := range req.GoodsInfo {
		...
		//for {
		mutex := global.RedSyncClient.NewMutex(fmt.Sprintf("goods_%d", goodInfo.GoodsId))
		...
		//这种写法有瑕疵，为什么？
		//当库存数量很少时进行抢购，会导致库存变为0，然后在下次更新时被gorm忽略掉
		//零值 对于int类型来说 默认值是0 这种会被gorm给忽略掉
		//使用select函数指定更新字段，0值也强制更新
...
func AutoReback(ctx context.Context, msgs ...*primitive.MessageExt) (consumer.ConsumeResult, error) {
	...
	for i := range msgs {
		...
		for _, orderGood := range sellDetail.Detail {
			//update怎么用
			//先查询一下inventory表， update语句的 update xx set stocks=stocks+2
